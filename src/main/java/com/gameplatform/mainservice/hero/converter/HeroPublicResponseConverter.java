...
public class HeroPublicResponseConverter {
    ...
    public List<HeroSimpleNameResponse> toSimpleNameList(List<HeroSimpleNameProjection> projections) {
    ...
    public HeroDetailsResponse toDetailsResponse(
        ...
        return new HeroDetailsResponse(
                ...
                new SimpleReferenceResponse(element.getId(), getLocalized(element.getNameJson(), locale)),
                new HeroRarityResponse(rarity.getId(), rarity.getStars()),
                new SimpleReferenceResponse(heroClass.getId(), getLocalized(heroClass.getNameJson(), locale)),
                family != null ? new SimpleReferenceResponse(family.getId(), getLocalized(family.getNameJson(), locale)) : null,
                new SimpleReferenceResponse(manaSpeed.getId(), getLocalized(manaSpeed.getNameJson(), locale)),
                alphaTalent != null ? new SimpleReferenceResponse(alphaTalent.getId(), getLocalized(alphaTalent.getNameJson(), locale)) : null,
    ...
    // Вспомогательный метод для локализации
    private String getLocalized(LocalizedTextJson json, String locale) {
        ...
        return switch (locale) {
            case "ru" -> json.ru();
            case "en" -> json.en();
            default -> json.ru(); // fallback на русский
        };
    }
}